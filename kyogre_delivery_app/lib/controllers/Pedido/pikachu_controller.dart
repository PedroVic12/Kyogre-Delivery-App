...
class PikachuController {
  ...
  void snackBarCarrinho(produto, BuildContext context) {
    showTopSnackBar(
      Overlay.of(context),
  ...
  void performActionWithPikachuData() {
    if (pikachuInfo.isNotEmpty) {
      print('Realizando ação com os dados de Pikachu: ${pikachuInfo}');
    } else {
  ...
  void carregamentoConcluido() {
    SnackBar(content: Text('Carregamento concluído!'));

    // snackbar('Rotinas Resetadas!', 'Tenha um otimo inicio de semana',
    //     showProgressIndicator: true,
    //     isDismissible: true,
